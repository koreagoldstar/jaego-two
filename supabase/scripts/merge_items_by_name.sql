...
begin
  ...
  for r in
    ...
    where
      (
        btrim(t.name) = '비상판넬(SH-EP500)'
        or (
          t.name ilike '%비상판넬%'
          and t.name ilike '%SH-EP500%'
          and t.name not ilike '%6.1%'
        )
      )
      and (
        btrim(s.name) = '비상판넬(SH-EP500)-6.1'
        or (
          s.name ilike '%비상판넬%'
    ...
    order by
      case when btrim(t.name) = '비상판넬(SH-EP500)' then 0 else 1 end,
      case when btrim(s.name) = '비상판넬(SH-EP500)-6.1' then 0 else 1 end
  loop
  ...
  if v_merged = 0 then
    raise exception '병합 대상 없음 — DB에 있는 비슷한 품목: %',
      coalesce((
        ...
        from public.items
        where name ilike '%비상판넬%' or name ilike '%EP500%'
      ), '(없음)');
  end if;
end $$;
...
where i.name ilike '%비상판넬%' or i.name ilike '%EP500%'
