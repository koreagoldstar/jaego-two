    ...
    <>
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-4 py-16 md:grid-cols-2 md:py-24">
          <div>
            ...
            <h1 className="mt-5 text-4xl font-extrabold leading-tight tracking-tight text-slate-900 md:text-5xl">
              바코드 한 번으로
              <br />
              끝나는 <span className="text-blue-600">오늘의 재고</span>
            </h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/signup"
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-3.5 font-semibold text-white shadow-sm hover:bg-blue-700"
              >
                {TRIAL_DAYS}일 무료로 시작하기 <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                href="/features"
                className="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-6 py-3.5 font-semibold text-slate-700 hover:bg-slate-50"
              >
                기능 둘러보기
              </Link>
            </div>
          ...
          <div className="relative">
            <div className="absolute inset-0 -z-10 mx-auto h-80 w-80 translate-y-10 rounded-full bg-blue-200/50 blur-3xl" />
            <PhoneMock />
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-slate-100 bg-white px-4 py-12">
        <div className="mx-auto grid max-w-5xl gap-4 text-center sm:grid-cols-3">
          {AUDIENCES.map(a => (
            <div key={a.title} className="rounded-2xl bg-slate-50 px-5 py-6">
      ...
      <section className="px-4 py-20">
        <div className="mx-auto max-w-6xl">
          <div className="text-center">
            <h2 className="text-3xl font-bold text-slate-900">재고관리에 필요한 건 다 있습니다</h2>
            <p className="mt-3 text-slate-600">실제 장비 업체 현장에서 매일 쓰며 다듬은 기능입니다.</p>
          </div>
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {mainFeatures.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="rounded-2xl border border-slate-200 p-6 transition-shadow hover:shadow-md">
                <div className="inline-flex rounded-xl bg-blue-600/10 p-2.5 text-blue-600">
          ...
          <div className="mt-10 text-center">
            <Link href="/features" className="inline-flex items-center gap-1 font-semibold text-blue-600 hover:underline">
      ...
      <section className="bg-slate-50 px-4 py-20">
        <div className="mx-auto max-w-5xl">
          <h2 className="text-center text-3xl font-bold text-slate-900">시작은 3단계면 충분합니다</h2>
      ...
      <section className="px-4 py-20">
        <div className="mx-auto max-w-5xl">
          <div className="text-center">
            <h2 className="text-3xl font-bold text-slate-900">품목 수에 맞춰 고르는 요금제</h2>
            <p className="mt-3 text-slate-600">모든 요금제에서 모든 기능을 씁니다. 직원·기기 수 제한도 없습니다.</p>
          </div>
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {PUBLIC_PLANS.map(p => (
              <div
                key={p.id}
                className={`rounded-2xl border p-6 ${p.highlight ? 'border-blue-600 ring-1 ring-blue-600' : 'border-slate-200'}`}
              >
          ...
          <div className="mt-8 text-center">
            <Link href="/pricing" className="inline-flex items-center gap-1 font-semibold text-blue-600 hover:underline">
      ...
      <section className="px-4 pb-4">
        <div className="mx-auto flex max-w-5xl flex-col items-start gap-6 rounded-3xl bg-slate-900 p-8 text-white md:flex-row md:items-center md:p-12">
          ...
          <div className="flex-1">
            ...
            <ul className="mt-4 flex flex-wrap gap-x-5 gap-y-1 text-sm text-slate-300">
              {['요구사항 무료 상담', '별도 견적', '기존 데이터 그대로'].map(t => (
                <li key={t} className="flex items-center gap-1.5">
                  <Check className="h-4 w-4 text-blue-400" /> {t}
                </li>
              ))}
            </ul>
          </div>
          <Link
            href="/custom"
            className="shrink-0 rounded-xl bg-white px-6 py-3.5 font-semibold text-slate-900 hover:bg-slate-100"
          >
            맞춤 제작 알아보기
          </Link>
        </div>
