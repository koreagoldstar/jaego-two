...
export default function ContactPage({
  ...
  return (
    <section className="bg-gradient-to-b from-blue-50 to-white px-4 py-16 md:py-20">
      <div className="mx-auto max-w-xl">
        <div className="text-center">
          <p className="text-sm font-semibold text-blue-600">문의하기</p>
          <h1 className="mt-3 text-3xl font-extrabold tracking-tight text-slate-900">무엇이든 물어보세요</h1>
          <p className="mt-3 text-slate-600">남겨 주신 연락처로 확인 후 빠르게 연락드립니다.</p>
        </div>

        {sent ? (
          <div className="mt-10 rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
            <CheckCircle2 className="mx-auto h-12 w-12 text-emerald-500" />
            <h2 className="mt-4 text-xl font-bold text-slate-900">문의가 접수되었습니다</h2>
            <p className="mt-2 text-slate-600">확인 후 남겨 주신 연락처로 연락드리겠습니다.</p>
            <Link href="/" className="mt-6 inline-block font-semibold text-blue-600 hover:underline">
              홈으로 돌아가기
            </Link>
          </div>
        ) : (
          ...
          >
            <fieldset>
              <legend className="mb-2 block text-sm font-medium text-slate-700">문의 종류</legend>
              <div className="grid grid-cols-3 gap-2">
                {KIND_OPTIONS.map(o => (
                  <label key={o.value} className="cursor-pointer">
                    <input type="radio" name="kind" value={o.value} defaultChecked={kind === o.value} className="peer sr-only" />
                    <span className="block rounded-xl border border-slate-200 px-2 py-2.5 text-center text-sm text-slate-600 peer-checked:border-blue-600 peer-checked:bg-blue-50 peer-checked:font-semibold peer-checked:text-blue-700">
                      {o.label}
                    </span>
                  </label>
                ))}
