...
export async function subtractItemQuantityLegacy(itemId: string, formData: FormData) {
  const supabase = await createClient()
  const {
    data: { user },
  } = await supabase.auth.getUser()
  ...
  const n = Math.max(1, parseInt(String(formData.get('subtract_qty') ?? '1'), 10) || 1)
  const { data: cur, error: selErr } = await supabase
    .from('items')
    .select('quantity')
    .eq('id', itemId)
  ...
  if (n > q) return { ok: false as const, error: `현재 재고는 ${q}개입니다` }

  const { error } = await supabase
    .from('items')
    .update({ quantity: q - n, updated_at: new Date().toISOString() })
    .eq('id', itemId)
    .eq('user_id', user.id)
